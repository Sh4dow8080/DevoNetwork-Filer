...
Citizen.CreateThread(function()
	...
	while true do
		...
		if((isNearFuelStation or isNearFuelPStation or isNearFuelHStation or isNearFuelBStation) and not IsPedInAnyVehicle(GetPlayerPed(-1))) then
			drawTxt('[~g~E~s~] for at købe en dunk (~b~50 $~w~)', 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
	...
	local isEngineOn = Citizen.InvokeNative(0xAE31E7DF9B5B132E, GetVehiclePedIsIn(GetPlayerPed(-1)))
		...
		if isEngineOn then
			if isRadarExtended then
				...
				DrawAdvancedText(0.1550, 0.9170, 0.185, 0.0028, 0.40,round(percent,1), 255, 255, 255, 255, 0, 1)
			elseif(hud_form == 1) then
				local percent = (essence/0.145)*100
				DrawAdvancedText(0.1650, 0.9170, 0.185, 0.0028, 0.40,round(percent,1), 255, 255, 255, 255, 0, 1)
			else
				local percent = (essence/0.145)*100
			end
			DrawAdvancedText(0.1335, 0.9170, 0.185, 0.0028, 0.40,"BENZIN ", 255, 255, 255, 255, 0, 1)
			DrawAdvancedText(0.1825, 0.9170, 0.185, 0.0028, 0.40,"%", 255, 255, 255, 255, 0, 1)
		end
    end
end
...
AddEventHandler("essence:hasBuying", function(amount, toPay)
	TriggerEvent("pNotify:SendNotification",{text = "Du har købt "..amount.." liter brændstof for <b style='color: #DB4646'>"..toPay.." $</b>",type = "success",timeout = (4000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
