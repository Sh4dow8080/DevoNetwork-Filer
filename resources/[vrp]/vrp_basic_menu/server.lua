                ...
                if nuser_id ~= nil then
                    ...
                    if math.random(1,3) == 1 then
                        if vRP.tryPayment({nuser_id,amount}) then
                            vRPclient.notify(nplayer,{"Overfaldte ~r~"..amount.."$."})
                            vRP.giveInventoryItem({user_id,"dirty_money",amount,true})
                        else
            ...
            if value ~= nil then
                ...
                if custom ~= nil then
                    if tonumber(custom) > 0 then
                        ...
                        vRPclient.setHandcuffed(target,{false})
                        vRPclient.teleport(target,{1708.1635742188,2594.5412597656,50.188068389893}) -- teleport inside jail
                                ...
                                if reason ~= nil and reason ~= "" then
                                    ...
                                    if target ~= nil then
                                        ...
                                        if payment ~= false then
                                            ...
                                            TriggerClientEvent("pNotify:SendNotification", player,{text = "Du gav en bøde på "..fine.." $ for: "..reason, type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                                            if payment == "paid" then
                                                TriggerClientEvent("pNotify:SendNotification", target,{text = "Du modtog en bøde på "..fine.." $ for: "..reason, type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                                            else
                    ...
                    if charge ~= nil and charge ~= "" then
                        ...
                        if target ~= nil then
                            vRP.getUserIdentity({user_id, function(identity)
                                ...
                                local text = "~b~" .. my_directory_name .. "~w~ kræver ~r~$" .. charge
                                vRP.request({target,text,600,function(req_player,ok)
                                    if ok then
                                        ...
                                        if target_bank>0 then
                                            ...
                                            TriggerClientEvent("pNotify:SendNotification", player,{text = "Mobilregning - Du blev betalt "..charge.. " $ af: "..directory_name, type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                                            TriggerClientEvent("pNotify:SendNotification", target,{text = ""..my_directory_name.." har modtaget "..charge.." $", type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            ...
            vRP.getUserByPhone({phone, function(target_id)
                ...
                if target_id~=nil then
                    if my_bank >= 0 then
                        ...
                        if target ~= nil then
                            vRP.setBankMoney({user_id,my_bank})
                            TriggerClientEvent("pNotify:SendNotification", player,{text = "Mobilepay - Du overførte "..transfer.." $ til: "..directory_name, type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}, sounds = { sources = {"mobilepay.ogg"}, volume = 0.6, conditions = {"docVisible"}}})
                            ...
                            vRP.getUserIdentity({user_id, function(identity)
                                ...
                                TriggerClientEvent("pNotify:SendNotification", target,{text = "MobilePay - Du modtog: "..transfer.." $ fra: "..my_directory_name, type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}, sounds = { sources = {"modtaget.ogg"}, volume = 0.5, conditions = {"docVisible"}}})
                            end})
                            vRP.closeMenu({player})
                        else
