...
AddEventHandler("vrp_panikknap:panik", function(coords)
    ...
    if vRP.hasPermission({user_id, cfg.permission}) then
        PerformHttpRequest('DIT_WEBHOOK_LINK', function(err, text, headers) end, 'POST', json.encode({username = "Panikknap logs", content = "ID: **"..user_id.."** har trykket panikknap"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler("vrp_panikknap:gps", function(coords)
    ...
    if vRP.hasPermission({user_id, cfg.permission}) then
        PerformHttpRequest('DIT_WEBHOOK_LINK', function(err, text, headers) end, 'POST', json.encode({username = "Panikknap logs", content = "ID: **"..user_id.."** har sendt gps"}), { ['Content-Type'] = 'application/json' })
