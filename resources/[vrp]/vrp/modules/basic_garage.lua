...
MySQL.createCommand("vRP/get_all_vehicles","SELECT * FROM vrp_user_vehicles INNER JOIN vrp_user_identities ON vrp_user_vehicles.user_id = vrp_user_identities.user_id WHERE vrp_user_identities.cpr LIKE @cpr AND vrp_user_identities.lastdigits = @last")
...
function vRP.getUserVehiclesByCPR(cpr, cbr)
    ...
    MySQL.query("vRP/get_all_vehicles", {cpr = search, last = split[2]}, function(rows, affected)
        if #rows > 0 then
            task({rows})
        else
            task()
            ...
            if usrList ~= "" then
                vRP.prompt(player,"Nærmeste spiller(e): " .. usrList .. "","",function(player,user_id)
                    ...
                    if user_id ~= nil and user_id ~= "" then
                        ...
                        if target ~= nil then
                            vRP.prompt(player,"Pris i $: ","",function(player,amount)
                                ...
                                if amount then
                                    if amount >= 0 then
                                            ...
                                            vRP.getUserIdentity(p_id, function(identityP)
                                                vRP.getUserIdentity(t_id, function(identityT)
                                                    ...
                                                    if #pvehicle > 0 then
                                                        ...
                                                        vRP.request(target,"<b>"..fornavnP.." "..efternavnP.."</b> ønsker at sælge sin <b>"..name.."</b> til en pris på <b style='color: #96D3FF'>"..amount.." $</b>", 10, function(target,ok)
                                                            if ok then
                                                                ...
                                                                if vRP.tryFullPayment(pID, amount) then
                                                                    ...
                                                                    TriggerClientEvent("pNotify:SendNotification", player,{text = {"Du solgte dit køretøj til "..fornavnT.." "..efternavnT.." for <b style='color: #4E9350'>"..amount.." $</b>."}, type = "success", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                                                    TriggerClientEvent("pNotify:SendNotification", target,{text = {"<b>"..fornavnP.." "..efternavnP.."</b> har solgt dig sit køretøj for <b style='color: #DB4646'>"..amount.." $</b>."}, type = "success", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})

                                                                    PerformHttpRequest('DIT_WEBHOOK_LINK', function(err, text, headers) end, 'POST', json.encode({username = "Salg af køretøj - Server " .. GetConvar("servernumber","0"), content = "**"..fornavnP.." "..efternavnP.." ("..p_id..")** solgte sit køretøj **"..name.."** til **"..fornavnT.." "..efternavnT.." ("..t_id..")** for **"..amount.." $**."}), { ['Content-Type'] = 'application/json' })
                                                                else
                                        ...
                                        TriggerClientEvent("pNotify:SendNotification", player,{text = {"Prisen skal være højere eller lig med 0 $."}, type = "error", queue = "global", timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                                    end
                                else
                ...
                if nuser_id ~= nil and nuser_id ~= "" then
                    ...
                    if target ~= nil then
                        ...
                        if mechlvl < 3 then
                            ...
                            PerformHttpRequest('DIT_WEBHOOK_LINK', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0").." - Kursus", content = dmessage}), { ['Content-Type'] = 'application/json' })
                        else
