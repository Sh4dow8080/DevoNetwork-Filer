      ...
      if business then -- have a business
            ...
            vRP.prompt(player,"Skriv id på personen der skal have firmaet:","",function(player,nuser_id)
              ...
              if target ~= nil then
                vRP.getUserBusiness(nuser_id, function(business)
                  if business then
                    TriggerClientEvent("pNotify:SendNotification", player,{text = {nuser_id.." har allerede en virksomhed!"}, type = "error", queue = "global", timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        ...
        menu[lang.business.open.title()] = {function(player,choice)
          if vRP.hasPermission(user_id,"advokat.service") then
            ...
            TriggerClientEvent("pNotify:SendNotification",player,{text = "Du skal være advokat for at kunne åbne virksomheder",type = "error",timeout = (3000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
          end
        end,lang.business.open.description({cfg.minimum_capital})}
      end
