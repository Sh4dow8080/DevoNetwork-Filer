...
MySQL.createCommand("vRP/get_money","SELECT wallet,bank,debt FROM vrp_user_moneys WHERE user_id = @user_id")
MySQL.createCommand("vRP/set_money","UPDATE vrp_user_moneys SET wallet = @wallet, bank = @bank, debt = @debt WHERE user_id = @user_id")
            ...
            if paid > 0 then
                if tmp.debt == 0 then
                    TriggerClientEvent("pNotify:SendNotification", source, {
                        text = "Du har indbetalt <b style='color: #4E9350'>"..format_thousand(math.floor(paid)).." $</b> til din gæld<br>Tillykke du har ikke mere gæld!",
                        type = "success",
                        timeout = (4000),
                        layout = "bottomCenter",
                        queue = "global",
                    ...
                    TriggerClientEvent("pNotify:SendNotification", source, {
                        text = "Du har indbetalt <b style='color: #4E9350'>"..format_thousand(math.floor(paid)).." $</b> til din gæld",
                        type = "success",
                        timeout = (4000),
                        layout = "bottomCenter",
                        queue = "global",
...
function vRP.setBankMoney(user_id,value)
    ...
    if tmp then
        if tmp.debt > 0 and tmp.bank < value then
            ...
            if tmp.debt > 0 then
                TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har betalt <b style='color: #4E9350'>"..format_thousands(payed).." $</b> til din gæld<br>Restgæld: <b style='color: #DB4646'>"..format_thousands(tmp.debt).." $</b>", type = "success", queue = "global", timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            else
                TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har betalt <b style='color: #4E9350'>"..format_thousands(payed).." $</b> til din gæld<br>Tillykke du har ikke mere gæld!", type = "success", queue = "global", timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
        else
            tmp.bank = value
        end
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
    ...
    if tmp and tmp.wallet ~= nil and tmp.bank ~= nil and tmp.debt ~= nil then
        MySQL.execute("vRP/set_money", {user_id = user_id, wallet = tmp.wallet, bank = tmp.bank, debt = tmp.debt})
    end
end)
...
AddEventHandler("vRP:save", function()
    for k,v in pairs(vRP.user_tmp_tables) do
        if v.wallet ~= nil and v.bank ~= nil then
            MySQL.execute("vRP/set_money", {user_id = k, wallet = v.wallet, bank = v.bank,debt=v.debt})
        end
    end
end)
