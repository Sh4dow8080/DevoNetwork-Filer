        ...
        if vRP.tryBankPayment({user_id,tonumber(data.money)}) then
          ...
          vRP.getUserIdentity({user_id, function(identity)
            ...
            TriggerClientEvent("pNotify:SendNotification", tosource,{
              text = "<h3>📱 Mobilepay</h3> <br> <p>Du har modtaget <b style='color:#64A664'>"..format_thousand(tonumber(data.money)).." $</b> fra <b style='color:#64A664'>"..fromname.."</b></p>",
              type = "info",
              timeout = (4000),
              layout = "bottomright",
              queue = "global",
          ...
          TriggerClientEvent("pNotify:SendNotification", source,{text = "<h3>📱 Mobilepay</h3> <br> <p>Dit kort er afvist</P>", type = "error", queue = "global", timeout = 4000, layout = "bottomright",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      else
        TriggerClientEvent("pNotify:SendNotification", source,{text = "<h3>📱 Mobilepay</h3> <br><p>"..display.." er utilgængelig", type = "error", queue = "global", timeout = 4000, layout = "bottomright",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    else
      TriggerClientEvent("pNotify:SendNotification", source,{text = "<h3>📱 Mobilepay</h3> <br><p>"..display.." er utilgængelig", type = "error", queue = "global", timeout = 4000, layout = "bottomright",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
