    ...
    if item ~= nil and item.price >= 0 then
        ...
        if vRP.tryFullPayment({nuser_id,tonumber(item.price)}) then
            ...
            if command ~= "" then
                ...
                TriggerClientEvent("pNotify:SendNotification", source,{text = "Køretøjet har fået "..item.title.."<br>"..((lvl ~= "off") and "Betalt: <b style='color: #DB4646'>"..item.price.."</b> $" or ""), type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            end
        else
            TriggerClientEvent("pNotify:SendNotification", source,{text = "Du har ikke råd til og købe opgraderingen", type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
        end
    end
end)
...
AddEventHandler('cn-mech:impoundveh', function(user_id,plate,veh,bool,netveh)
    ...
    if bool == 0 then
        ...
        TriggerClientEvent("pNotify:SendNotification", source,{text = veh.." "..plate.." er blevet frigivet!", type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    elseif bool == 1 then
        local nuser_id = vRP.getUserId({source})
        vRP.prompt({source, firstToUpper(veh).." ("..plate..") beslaglagt årsag:", "", function(player,reason)
            if string.len(reason) >= 2 and string.len(reason) <= 200 then
                ...
                TriggerClientEvent("pNotify:SendNotification", player,{text = "<b style='color: #4E9350'>"..firstToUpper(veh).." ("..plate..")</b> er blevet beslaglagt!<br/>Årsag: <b style='color: #4E9350'>"..reason.."</b>", type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                exports["vrp_garages"]:updateGarage(user_id)
                sendToDiscord("Server "..GetConvar("servernumber", "0").." - Beslaglagt","**"..nuser_id.."** har lige beslaglagt **"..firstToUpper(veh).."** fra **"..user_id.."** (**"..os.date("%H:%M:%S %d/%m/%Y").."**)")
                vRPct.despawnNetVehicle(player,{netveh})
            else
                TriggerClientEvent("pNotify:SendNotification", player,{text = "Du skal beskrive en grund", type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            end
        end})
    end
end)
