...
local animations = {
    ...
    ['notesblok'] = {dict = "amb@medic@standing@timeofdeath@base", anim = "base", locked = false, prop = {propn = "p_notepad_01_s", bindex = 0xeb95}, options = {8.0, 1.0, -1, 49, 0},keepplaying=true, blocked = true},
...
Citizen.CreateThread(function()
    while true do
        ...
        --     end
        -- end
        if IsControlJustPressed(1, 74) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedUsingScenario(GetPlayerPed(-1), "WORLD_HUMAN_SUNBATHE_BACK") then
        ...
        if IsControlJustPressed(1, 113) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedUsingScenario(GetPlayerPed(-1), "WORLD_HUMAN_SUNBATHE_BACK") then -- INSERT
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/tis', 'Skal bruges når du skal pisse, ellers pisser du i bukserne!')
    TriggerEvent('chat:addSuggestion', '/me', 'Denne funktion bruges til at skrive f.eks en tanke. (/me Tænker: Nøj jeg kunne godt en pizza.)')
    ...
    TriggerEvent('chat:addSuggestion', '/notesblok', 'Tager en notesblok frem (Kræver en notesblok).')
